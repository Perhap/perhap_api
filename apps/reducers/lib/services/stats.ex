...
defmodule Service.Stats do
  ...
  @domains [:stats]
  @orderable false
  @types [:bin_audit, :actuals, :pre_actual, :refill_actual, :pre_challenge, :refill_challenge, :challenge]
  ...
  def validate(event_list) do
    ...
    |> Enum.map( fn(event) -> IO.inspect(event) end)
    # |> sorts_events()
  end

  def bin_audit_prep(meta)do
    meta
    |> remove_nulls("DATE")
    |> filter_on_date("DATE")
  end

  def challenge_prep(meta)do
    meta
    |> filter_on_date("start_date")
  end

  def actuals_prep(meta)do
    meta
    |> remove_nulls("Count")
    |> filter_on_date("Week")
  end
  ...
  def filter_on_date(data_list, date_field_name)do
    ...
    {start_name, start_times} = hd(time_periods)
    start_time = start_times.start_time

    {end_name, end_times} = List.last(time_periods)
