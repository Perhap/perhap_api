defmodule Mix.Tasks.Stats.Dups do
  use Mix.Task

  def fix_dups(state, store_num) do
    week_stats = state["stats"]
    stats = Map.new(Enum.map(week_stats, fn {name, week} -> dedup({name, week}, store_num) end))
    Map.put(state, "stats", stats)
  end

  def dedup({name, week}, store_num) do
